...
Param.search.s_range = [0.1:0.1:0.9 0.995];
Param.search.c_range = [0.1:0.1:0.9 0.995];
Param.search.m_range = [0.1:0.1:0.9 0.91:0.01:0.99 0.995 0.999 0.9995];
...
data = data(1:5000, :);
disp('Grid Search is done part of the data = 1000')

%% Perform grid search 
Param.dist = dist{testBatchSize}; 
Param.pdf_option= pdf_function{trainBatchSizeRatio};
[ best_param ] = gridSearch_noC_DU_2s(data, Param );
% Write the results 
output_file = sprintf('bp_noC_DU_2s_%s_%s.mat', Param.dist, Param.pdf_option);
