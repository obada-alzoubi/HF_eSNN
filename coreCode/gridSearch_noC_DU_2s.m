function [ best_param ] = gridSearch_noC_DU_2s( data, Param )
...
%upSample = max(data);
%downSample = min(data);
%data = [upSample; downSample];
for i=1:gridSize
    
    fprintf('iteration %d \n', i);
    % Train
    [ repos, ~, ~] = findWeight(data, Params{i});
    % Test
    num_clus = length(unique(repos.label));
    s =dunns(num_clus, repos.w, repos.label);
    mean_s = mean(s);
